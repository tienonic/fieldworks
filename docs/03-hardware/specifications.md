...
A specification is usable only when the exact physical model matches it. Values marked `design gate` must be rechecked against the physical label and final selected model.
...
| Control | short open/close pulses, never sustained coil power |
...
| ENTS path | GPIO command to an isolated transistor/H-bridge/DPDT relay driver, not directly to the solenoid |
| Acceptance | five consecutive open/close cycles at low battery voltage without reset or overheating |

The final driver cannot be selected until the exact solenoid datasheet is attached to the station record.
...
| WQ-44 internal space | approximately 156 × 110 × 79 mm in the compatibility record |
...
| Sub-band | must match every node; sub-band 2 is an assumption until configured and tested |
| Acceptance | all seven ENTS nodes join, uplink, reconnect, and preserve unique station identity |

## MET-01 baseline specifications

These are wiring baselines, not verified inventory.
...
No exact specifications are available for most proposed instruments. The candidate record therefore stores the required selection fields rather than invented values.
